...
export const networkProviders: { [chainId: string]: providers.JsonRpcProvider } = NETWORKS.reduce(
	(networks, network) => {
			...
			providerInstance.request({ method: 'eth_requestAccounts' }).then((accounts: string[]) => {
				if (accounts.length > 0) {
					...
					const supportedNetwork: boolean =
						NETWORKS.filter((network) => network.chainId === chainId).length > 0;
					if (supportedNetwork)
						selectedNetworkIndex.set(NETWORKS.findIndex((network) => network.chainId === chainId));
					accountChainId.set({ chainId, supportedNetwork });
					walletAddress.set(accounts[0]);
					accountsChangedListener = (accounts: string[]) => {
						if (accounts.length > 0) walletAddress.set(accounts[0]);
